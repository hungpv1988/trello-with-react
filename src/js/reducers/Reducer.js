...
const initialState = [{id: 1, name: 'Planing', status: ItemStatus.Backlog}]
...
export default function todoAppReducer(state = initialState, action)
{
    switch(action.type)
    {
        case types.ADDNEW: 
            ...
            return [...state, {id: state[state.length-1].id+1, name: action.text, status: action.status}];

        case types.EDIT:
            return state.map((item, index) => (item.id == action.todo.id) ? {...item, name:action.todo.name, status:action.todo.status} : item )

        case types.NEXTSTATUS:
            return state.map((item, index) => (item.id == action.todo.id) ? {...item, status:action.todo.status} : item );
